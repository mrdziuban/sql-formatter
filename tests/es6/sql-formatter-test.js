...
import formatSql from '../../es6/sql-formatter';
...
describe('formatSql', () => {
  describe('formatting of tabbed keywords', () => {
    tabbedKeywords.forEach((word) => {
      it(`formatting of '${word}'`, () => assert.equal(formatSql(`foo ${word} bar`, 2), `foo\n  ${word} bar`));
    });
  });
  ...
  describe('formatting of unchanged keywords', () => {
    unchangedKeywords.forEach((word) => {
      it(`formatting of '${word}'`, () => assert.equal(formatSql(`foo ${word} bar`, 2), `foo ${word} bar`));
    });
  });

  describe('SELECTs', () => {
    it("formatting of 'SELECT'", () => assert.equal(formatSql('SELECT foo bar', 2), 'SELECT foo bar'));
    it("formatting of ' SELECT'", () => assert.equal(formatSql(' SELECT foo bar', 2), 'SELECT foo bar'));
    it("formatting of '(SELECT'", () => assert.equal(formatSql('foo (SELECT bar', 2), 'foo\n  (SELECT bar'));
    it("formatting of '( SELECT'", () => assert.equal(formatSql('foo ( SELECT bar', 2), 'foo\n  (SELECT bar'));
    it("formatting of ') SELECT'", () => assert.equal(formatSql('foo) SELECT bar', 2), 'foo)\nSELECT bar'));
    it("formatting of ')SELECT'", () => assert.equal(formatSql('foo)SELECT bar', 2), 'foo)\nSELECT bar'));
    it("Formatting when selecting multiple fields", () => assert.equal(formatSql('SELECT foo, bar, baz', 2), 'SELECT foo,\n    bar,\n    baz'));
  });

  describe('UPDATEs', () => {
    it("formatting of 'UPDATE'", () => assert.equal(formatSql('UPDATE foo bar', 2), 'UPDATE foo bar'));
    it("formatting of ' UPDATE'", () => assert.equal(formatSql(' UPDATE foo bar', 2), 'UPDATE foo bar'));
  });

  describe('DELETEs', () => {
    it("formatting of 'DELETE'", () => assert.equal(formatSql('DELETE foo bar', 2), 'DELETE foo bar'));
    it("formatting of ' DELETE'", () => assert.equal(formatSql(' DELETE foo bar', 2), 'DELETE foo bar'));
  });

  describe('special case keywords', () => {
    it("formatting of 'THEN'", () => assert.equal(formatSql('foo THEN bar', 2), 'foo THEN\n  bar'));
    it("formatting of 'UNION'", () => assert.equal(formatSql('foo UNION bar', 2), 'foo\nUNION\nbar'));
    it("formatting of 'USING'", () => assert.equal(formatSql('foo USING bar', 2), 'foo\nUSING bar'));
  });
  ...
  describe('case transformations', () => {
    [].concat.apply(
      ...
      it(`upcasing of '${word}'`, () => assert.equal(formatSql(` ${word.toLowerCase()} `, 2).trim(), word));
    });
  });
